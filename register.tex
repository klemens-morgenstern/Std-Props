...
\section{Problem}
In current C++, there is not clear way how to access a fixed structure of bits,
if there is no standard type for that. This problem needs a solution, which allows
the programmer to access a sequence of bits which should be modeled as different
values with different types. This is especially necessary for embedded systems. 
Very view controllers have a low-level API which wraps around the hardware in a 
safe and perfomant matter, so much of this code has to be written. At the moment there are two used solutions
for this problems.
...
communication, where reducing the bit-size is crucial.
...
makes the bitfield utterly useless for hardware-access, if no additional
...
Therefore there is no given guarantee for any alignment and the existance of a bitfield has no effect on the struture. 
It is basicly just a way to express, that a member may be smaller than it's
type.
This is confusing and inconsistent, since no member-pointers can be used for the
bitfields which behave like regular members in all other use cases. 
...
For a volatile access of the register, only one access is allowed, be it for
...
This mechanic is needed, when reading a hardware register, because it may change
anytime. That is, a way to obtain consistent data is necessary. C++
provides atomics in the form of a library, but they would be rather hard to use
with custom types.
Additionally, the intent of volatile is exactly the one described here.
\subsection{Underlying Type}
The underlying type maybe statet explicitly, like for strongly-typed
enumerators.
...
{0b11111101}.
This is similar to the current implementation of bitfields.
...
not the representation on the machine itself.
...
\end{itemize} 
Which of course has the C++-representation of 0xBEEF in both cases. 
...
reg r2 { .i = 42, .k = 3};
\end{lstlisting}
In the latter case, all not-initialized members are zero.
...
pointers or references. In an later addition this might be implemented via a
...
\subsection{Templated Registers}
...
\subsection{Current Solution}\label{sec:fun_sol}
...
Every other solution would also just be based upon those sorts of function,
though it may wrap them into a class and/or template.


Having a register solution, which´s correctness
can be checked by a static\_assert is therefore a huge improvement.
