...
\subsection{Function Solution}\label{sec:fun_sol}
...
\subsection{Object Solution}
Now it might seem, that we could use a class construction, but this comes with a
few problems. In order to access the value we can put it into a member-variable,
...
\end{lstlisting} 
The first way to implement this is to use the functions given in
\ref{sec:fun_sol} as member-functions. This would be similar and also have the
problem of being very verbose and error-prone.\\
A better solution would be to implement the member-access via a
property-class template. The problem here is that such a class template would
need to hold a reference to the actual value. This would require unnecessary
memory and introduce an additional indirection.\\
To solve that, we could pass a pointer to the required address as a template
parameter. This would be fast, but then the struct could not be copied.\\
A last and very strange solution is, to put the register value inside a union
with the properties. Then the properties can cast the this-pointer and access
this to the register value. This would work, since the values of the union all
exist in the same memory. This is however a bad solution and requires a lot of
\lstinline {reinterpret_cast}s.
